...
<main>
  ...
  <c:if test="${!empty admin}">
    ...
    <c:choose>
      <c:when test="${empty allPersons}">
        <p>Il n'y a pour le moment aucun utilisateur sur cette application.</p>
      </c:when>
      <c:otherwise>
        <ul>
          <c:forEach var="person" items="${allPersons}">
            <li>${person.firstName} ${person.name} - <a href="<c:url value=" deleteUser?id=${person.id}" />">Supprimer</a></li>
          </c:forEach>
        </ul>
      </c:otherwise>
    </c:choose>
